...
def compute_loss(predictions, targets, model, reduction="mean"):
    ...
    if reduction == "mean":
        ...
        lcls, lbox, lobj = torch.zeros(predictions[0].shape[0], device=device), \
                           torch.zeros(predictions[0].shape[0], device=device), \
                           torch.zeros(predictions[0].shape[0], device=device)
    ...
    BCEcls = nn.BCEWithLogitsLoss(
        pos_weight=torch.tensor([1.0], device=device), reduction=reduction)
    ...
    for layer_index, layer_predictions in enumerate(predictions):
        ...
        if num_targets:
            ...
            if reduction == "mean":
                lbox += (1.0 - iou).mean() * 0.05  # iou loss
            else:
                lbox += [(1.0 - iou) * 0.05]
            ...
            if ps.size(1) - 5 > 1:
                ...
                if reduction == "mean":
                    ...
                    lcls += [BCEcls(ps[:, 5:], t).mean(dim=[1, 2, 3]) * 0.5]
        else:
            if reduction is not "mean":
                lcls += torch.tensor([0.]*layer_predictions.shape[0], device=device)
                lbox += torch.tensor([0.]*layer_predictions.shape[0], device=device)
        ...
        if reduction == "mean":
            ...
            lobj += BCEobj(layer_predictions[..., 4], tobj).\
                        mean(dim=[1, 2, 3]) * 1.0  # obj loss

    # Merge losses
    # if reduction is "mean":
    loss = lbox + lobj + lcls
    # else:
    #     loss = torch.cat([x + y + z for x, y, z in zip(lobj, lbox, lcls)])
    #     lbox = torch.cat(lbox)
    #     lobj = torch.cat(lobj)
    #     lcls = torch.cat(lcls)
