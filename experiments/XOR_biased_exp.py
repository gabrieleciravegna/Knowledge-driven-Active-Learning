...
for seed, (train_idx, test_idx) in enumerate(skf.split(x_t.cpu(), y_t.cpu())):
    for strategy in strategies:
        ...
        ### NEED TO CREATE A BIASED DATASET AT FIRST: X1
        # first_idx = np.random.choice(train_sample, first_points, replace=False).tolist()
        idx_quad_1 = torch.argwhere(x_t[:, 0] < 0.5)
        idx_quad_1 = idx_quad_1[torch.randperm(idx_quad_1.shape[0])]
        first_idx = idx_quad_1[:first_points].squeeze().numpy().tolist()
        print("First idx", first_idx)

        sns.scatterplot(x=x_t[:, 0].numpy(), y=x_t[:, 1].numpy(), hue=y_t.numpy())
        plt.scatter(x=x_t[first_idx, 0].numpy(), y=x_t[first_idx, 1].numpy(), c="red")
